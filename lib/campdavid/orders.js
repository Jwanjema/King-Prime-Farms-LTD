...
export async function getUserOrdersForDate(token, date) {
  const { ok, data } = await campdavidFetch("/user-orders", { body: { date }, token });
  return ok && Array.isArray(data) ? data : [];
}

export async function cancelOrder(token, { orderId, reason }) {
  return campdavidFetch("/cancelOrder", { body: { order_id: orderId, reason }, token });
}
